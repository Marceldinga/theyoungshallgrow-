# dashboard_panel.py ✅ UPDATED (canonical dashboard_next_view + financial summary support-ready)
...
def render_dashboard(sb_anon, sb_service, schema: str):
    ...
    st.caption(f"Session UUID: {session_id or 'N/A'} • Rotation start: {format_date(rotation_start_date)}")
    ...
    if current_pot in (None, ""):
        alerts.append("Pot amount is not available (dashboard_next_view missing or unreadable).")
    else:
        try:
