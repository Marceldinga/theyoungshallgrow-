...
def render_member_request_loan_tab(
    c,
    schema: str,
    profile,
    user_email: str,
    member_labels: list[str],
    label_to_id: dict,          # ✅ was label_to_legacy_id
    label_to_name: dict,
    signature_box,
    show_api_error,
    actor_user_id: str,
    ...
    with col1:
        ...
        surety_id = int(label_to_id[surety_pick])   # ✅ no legacy
    ...
    if st.button("Create Loan Request", use_container_width=True, key="loan_req_create_btn"):
        try:
            # If your create_loan_request requires schema, add it here.
            ...
            # fallback if your create_loan_request doesn't accept schema param
            try:
                req_id = create_loan_request(
                    c,
                    requester_user_id=str(profile["id"]),
    ...
    try:
        req_rows = (
            c.schema(schema).table("loan_requests")
    ...
    st.divider()
    st.markdown(
        f"### Latest Request: **ID {req_id}** • status={req.get('status')} • amount={float(req.get('amount') or 0):,.0f}"
    )

    st.markdown("### Required Signatures (Loan)")
    signature_box(
        c, "loan", req_id, "borrower",
    ...
    signature_box(
        c, "loan", req_id, "treasury",
        default_name=user_email,
        signer_member_id=requester_member_id,
        key_prefix="loan_sig",
    )

    # ✅ payout.py new signature expects schema
    sig_rows = get_signatures(c, schema, "loan", req_id)
    miss = missing_roles(sig_rows, LOAN_SIG_REQUIRED)
